...
- **Be Explicit**: Favor explicit variable names over concise ones (e.g., `postgres_connection_string` or `database_dsn` instead of just `dsn` or `url`).
- **Triple-Quoted Docstrings**: Use triple-quoted strings for complex Python functions/classes. Document inputs, outputs, and expected shapes (e.g., for embeddings: `(512,)` float32).
- **Context**: Explain *what* a table or configuration is for, not just that it exists.
