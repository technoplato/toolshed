    ...
    try {
      ...
      if (path === "/health" && method === "GET") {
        return Response.json(
          { status: "ok", appId: APP_ID.slice(0, 8) },
          { headers }
        );
      }
      ...
      if (path === "/videos" && method === "GET") {
        ...
        if (sourceId) {
          videos = videos.filter((v: any) => v.source_id === sourceId);
        }
      ...
      if (path.startsWith("/videos/") && method === "PUT") {
        ...
        await db.transact(tx.videos[videoId].update(updateData));
      ...
      if (path === "/ingestion-runs" && method === "POST") {
        ...
        if (video.source_id) {
          ...
          if (existing.videos && existing.videos.length > 0) {
            ...
            console.log(
              `Found existing video for source_id=${video.source_id}: ${videoId}`
            );
          } else {
            videoId = id();
            isNewVideo = true;
        ...
        try {
          ...
          if (isNewVideo) {
            ...
            const updateData: any = { ingested_at: timestamp };
          ...
          if (transcriptionRun) {
            await db.transact(
              tx.transcriptionRuns[transRunId]
                .update({
                  tool_version: transcriptionRun.tool_version || "mlx-whisper",
                  pipeline_script:
                    transcriptionRun.pipeline_script || "audio_ingestion.py",
                  is_preferred: transcriptionRun.is_preferred ?? true,
                  input_duration_seconds:
                    transcriptionRun.input_duration_seconds,
                  processing_time_seconds:
                    transcriptionRun.processing_time_seconds,
          ...
          if (diarizationRun) {
            await db.transact(
              tx.diarizationRuns[diarRunId]
                .update({
                  workflow: diarizationRun.workflow || "pyannote",
                  tool_version:
                    diarizationRun.tool_version || "pyannote-audio-3.1",
                  pipeline_script:
                    diarizationRun.pipeline_script || "audio_ingestion.py",
                  ...
                  processing_time_seconds:
                    diarizationRun.processing_time_seconds,
      ...
      if (path === "/diarization-segments" && method === "POST") {
        ...
        if (!run_id || !segments || !Array.isArray(segments)) {
          return Response.json(
            { error: "run_id and segments array required" },
            { status: 400, headers }
          );
        }
        ...
        return Response.json(
          { success: true, count: segmentIds.length, segment_ids: segmentIds },
          { headers }
        );
      }
      ...
      if (path === "/words" && method === "POST") {
        ...
        if (!run_id || !words || !Array.isArray(words)) {
          return Response.json(
            { error: "run_id and words array required" },
            { status: 400, headers }
          );
        }
        ...
        return Response.json(
          { success: true, count: wordIds.length },
          { headers }
        );
      }
      ...
      if (path === "/diarization-segments" && method === "GET") {
        ...
        if (runId) {
          ...
          if (startTime !== null) {
            segments = segments.filter(
              (s: any) => s.end_time >= parseFloat(startTime!)
            );
          }
          if (endTime !== null) {
            segments = segments.filter(
              (s: any) => s.start_time <= parseFloat(endTime!)
            );
          }
        ...
        return Response.json(
          { error: "video_id or run_id required" },
          { status: 400, headers }
        );
      }
      ...
      if (path === "/speaker-assignments" && method === "POST") {
        ...
        return Response.json(
          { success: true, count: assignments.length, result },
          { headers }
        );
      }
      ...
      if (path === "/speakers" && method === "POST") {
        ...
        if (existing.speakers.length > 0) {
          return Response.json(
            { speaker: existing.speakers[0], created: false },
            { headers }
          );
        }
        ...
        return Response.json(
          { speaker: result.speakers[0], created: true },
          { headers }
        );
      }
