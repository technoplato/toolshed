...
class ASRDiarizationPipeline:
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        diarization_pipeline = Pipeline.from_pretrained(diarizer_model, token=use_auth_token)
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # Pyannote 4.0 compatibility
        if hasattr(diarization, 'speaker_diarization'):
             # This is likely a DiarizeOutput object from pyannote 4.0
             ...
             iterator = diarization.annotation.itertracks(yield_label=True)
        else:
             # Try to find anything that looks like tracks
             print("DEBUG: Could not find itertracks, trying fallback")
             iterator = []

        segments = []
        for segment, track, label in iterator:
            segments.append({
                'segment': {
                    'start': segment.start,
                    'end': segment.end
