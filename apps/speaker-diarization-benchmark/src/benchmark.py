...
# Configure logging
# Add apps/transcriber to path to import instant_logger
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '../../../apps/transcriber')))
try:
    ...
    if not APP_ID or not ADMIN_TOKEN:
        logger.warning("Missing INSTANT_APP_ID or INSTANT_ADMIN_TOKEN. InstantDB logging disabled.")
        # Fallback to standard logging if credentials missing
        raise ImportError("Missing credentials")

    logger = setup_instant_logging(APP_ID, ADMIN_TOKEN, source="benchmark")
    logger.info("InstantDB logging initialized")
except ImportError:
    ...
    logger.warning("Could not import instant_logger. Using standard logging.")
